---
name: comm-brief
description: 重流程——从飞书消息出发给群/人做 memory 分析：拉 lark-cli 消息 → AI 分析出候选观察 → 主动问用户"你有啥要一起加的认知" → 合并口述和消息观察 → 用户审+改+落。触发：用户只给了对象没给描述，比如"给 Hayden 建档"/"xx 群最近啥情况"/"给 xx 群建档"/"最近半个月 xx 群有什么变化"/"看跟 X 最近互动"/"今天谁 @ 我了"。如果用户已经把完整口述讲出来了（"X 是 xxx 风格，雷区是 xxx"），走 comm-memory 而不是本 skill。
