...
| Skill | 驱动源 | 主任务 | 读/写 memory |
|---|---|---|---|
| **reply-coach** | 用户说"帮我回这条" | 起草回复建议（柔和推进 / 就事论事 / 强势推回三种策略） | **只读** |
| **comm-brief** | 消息（lark-cli 拉到的） | 从消息里提炼**变化观察** → 候选写入群/人档案 | 读 + 用户确认后写 |
| **comm-memory** | 用户口述意图 | 档案 CRUD（看/建/改人物、群、个人画像） | 读 + 用户确认后写 |

**分水岭**：谁驱动？
- 用户说"帮我回"→ `reply-coach`
- 消息流驱动（"xx 群最近啥情况"）→ `comm-brief`
- 用户直接管档案（"记一下"、"看看 X 档案"）→ `comm-memory`
