...
不是一个大 skill，而是三个独立 skill 各有明确触发场景，共享 memory / references / prompts：

```
<repo root>/
└── skills/
    ├── reply-coach/SKILL.md       # 回复参谋：分析 + 策略 + 回复建议（只读 memory）
    ├── comm-memory/SKILL.md       # 记忆管理：查看/编辑/新建 档案（读写 memory）
    ├── comm-brief/SKILL.md        # 沟通简报：@汇总/群动态/人物回顾（只读 memory）
    └── shared/
        ├��─ references/
        ...
        └── memory/
            ...
            │   └── group-profile.md
            ├── index.md                # 索引 + 设计原则
            ├── user-profile.md         # 用户画像（始终加载）
            ├── persons/{person}.md     # 人物档案
            └── groups/{group}.md       # 群组上下文
```

**关键分工：**
- **reply-coach** 只读 memory，不写——回复辅助和记忆管理解耦
- **comm-memory** 读写 memory——用户明确要记录或修改时才触发
- **comm-brief** 只读 memory + 调 lark-cli 拉消息——信息汇总，不分析不建议

## Memory 系统

**设计原则（跟 Claude Code 的 memory.md 同一思路）：**

- **AI + 用户共同维护。** 两者同等权威，混在同一文件里，不区分来源。
- **三层加载。** `user-profile.md` 始终 → `groups/{group}.md` 按群 → `persons/{person}.md` 按人。
- **人档案记稳定特征 + 互动模式，群档案记场景特征。** 同一人的"多面性"通过群档案可索引。
- **渐进累积，可被推翻。**
